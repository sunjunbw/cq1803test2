...
1. 下载对应工具：node-sass sass-loader
...
更改state的时候需要调用mutations，但是mutations中的函数不能出现异步的动作,因为这样会导致mutations维护困难，因为mutations应该是纯粹的，只是做一些更改state的动作的一个工具，不要在其中加入一些其他的相关异步动作
